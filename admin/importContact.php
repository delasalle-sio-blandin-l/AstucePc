...
$fichier = $_FILES["userfile"]["name"];
    ...
    while (!feof($fp))
    {
        ...
        if ($champs1!='')
        {
            ...
            $req_pre = $cnx->prepare("INSERT INTO `contact`(`contactRef`, `nom`, `prenom`, `telephone`, `mail`) 
            VALUES (:ref, :nom, :prenom, :telephone, :mail)");
            $req_pre->bindValue(':ref', $champs1, PDO::PARAM_STR);
            $req_pre->bindValue(':nom', $champs2, PDO::PARAM_STR);
		    $req_pre->bindValue(':prenom', $champs3, PDO::PARAM_STR);
            $req_pre->bindValue(':telephone', $champs4, PDO::PARAM_STR);
            $req_pre->bindValue(':mail', $champs5, PDO::PARAM_STR);
            $req_pre->execute();
...
<h4>Nombre de valeurs nouvellement enregistrées : </h4><?php echo $cpt;?>
