<?php
	...
	if (isset($_GET['modifier_actualite'])) 
	{
		...
		$req_pre = $cnx->prepare("SELECT * FROM actualites WHERE actuRef = :ref");
		// Variable lié à la requête SQL
		$req_pre->bindValue(':ref', $_GET['modifier_actualite'], PDO::PARAM_INT);
		$req_pre->execute();
		//le résultat est récupéré sous forme d'objet
		$liste=$req_pre->fetch(PDO::FETCH_OBJ);
		
		// On place les données dans des variables.
		$texte = $liste->texteActu;
		$ref = $liste->actuRef;
	}

	require('affichageModifActualite.php');
?>